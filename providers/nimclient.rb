...
action :allocate do
  ...
  unless lpp_source.nil?
    if resource_exists(lpp_source)
      if ! is_resource_allocated(lpp_source,"lpp_source")
  ...
  unless spot.nil?
    if resource_exists(spot)
      if ! is_resource_allocated(spot,"spot")
  ...
  unless installp_bundle.nil?
    if resource_exists(installp_bundle)
      if ! is_resource_allocated(installp_bundle,"installp_bundle")
...
action :cust do
  ...
  lpp_source=@new_resource.lpp_source
  unless lpp_source.nil?
    if lpp_source=="next_sp" or lpp_source=="next_tl" or lpp_source=="latest_tl" or lpp_source=="latest_sp" 
  ...
  unless fixes.nil?
    nimclient_s=nimclient_s << " -a fixes=\"" << fixes << "\""
  end
  ...
  if fixes=="update_all"
    ...
    if "#{lpp_source_oslevel}"=="#{current_oslevel}"
      do_not_converge=true
    end
  end
    ...
    showres.each_line do |s_fileset|
      if s_fileset.include? "#{a_fileset}"
        ...
        if s_fileset.include? "_all_filesets"
          ...
          if ! all_filesets.include? get_fileset
            all_filesets.push(get_fileset)
          end
        end
      end
    end 
