...
vios_input = STDIN.readline.chomp
vios_list = vios_input.to_s.strip.split()
tuple = '(' + vios_list[0].to_s
tuple = tuple + ',' + vios_list[1].to_s if vios_list.length > 1
tuple += ')'
...
disks = disks + ',' + hdisk_list[1].to_s if hdisk_list.length > 1
disks += ')'
