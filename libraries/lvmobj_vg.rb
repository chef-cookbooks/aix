    ...
    def check_to_change
      ...
      for current_pv in @physical_volumes
        ...
        if !current_vg.nil? && current_vg != @name
          raise AIXLVM::LVMException, format('physical volume "%s" is use in a different volume group!', current_pv)
        end
      end
      @current_physical_volumes = []
    ...
    def create
      ...
      if @changed
        ...
        if @current_physical_volumes.empty?
          ...
          ret.push(format("Create volume groupe '%s'", @name))
          vg_obj.modify('y') if @use_as_hot_spare == 'y'
          ret.push(format("Extending '%s' to '%s'", @physical_volumes[0], @name))
